...
def hybrid_recommendation(user_id, input_title_or_genre, K=5, collaborative_weight=0.5):
    ...
    # Combine scores
    hybrid_scores = (collaborative_weight * collaborative_score) + \
        ((1 - collaborative_weight) * content_based_movies.index)

    # Get the top K movies based on hybrid scores
    top_hybrid_movies = movies_data['title'].iloc[hybrid_scores.argsort()[
        ::-1][:K]]
...
if st.button("Get Recommendations"):
    try:
        recommended_movies = hybrid_recommendation_function(
            user_id, user_input)
